...
describe('validator', () => {
  describe('not over energy output', () => {
    ...
    describe(`base is ${JSON.stringify(baseAssembly)}`, () => {
      describe.each([
        {core: cores.find(c => c.name.includes('JAILBREAK'))!, expected: false},
        {core: cores.find(c => c.name.includes('NACHTREIHER'))!, expected: false},
        {core: cores.find(c => c.name.includes('VP-40S'))!, expected: true},
      ])('when core is $a', ({core, expected}) => {
        it(`should return ${expected} by isSuccess`, () => {
          expect(notOverEnergyOutput.validate({...baseAssembly, core}).isSuccess).toBe(expected)
        })
      })
    })
  })
